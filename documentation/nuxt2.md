...
![TwicPics Components](https://raw.githubusercontent.com/twicpics/components/0.26.0/documentation/resources/nuxt-cover.png)
...
`TwicImg`, `TwicPicture`, and `TwicVideo` are available in your templates as long as you have [configured the TwicPics Nuxt2 Module](#setting-up-twicpics-components-into-your-nuxt2-project).

Use them in your template files instead of `img`, `picture`, or `video` tags (see [Components Properties](#components-properties)).
...
To achieve _art direction_, configure the following `TwicPicture`'s properties according to the **mobile-first principle**:
...
Helpful if you want to display an image with its intrinsic aspect ratio without cropping.
