...
If you want to display a __content pixel-perfect image__ with optimized _Cumulative Layout Shift_ (CLS), _Low-Quality Image Placeholder_ (LQIP) and lazy loading out of the box, you can use the `<twic-img>` component.
...
For seamless playback of [videos optimized with TwicPics](https://www.twicpics.com/docs/topics/video-optimization), use the `<twic-video>` component. It's a sibling of `<twic-img>` and serves as a drop-in replacement for the standard `video` tag.
...
`<twic-img>` and `<TwicPicture>` components allow to __reframe__ your image on the __main subject(s)__ they contain.
