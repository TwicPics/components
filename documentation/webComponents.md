...
![TwicPics Components](https://raw.githubusercontent.com/twicpics/components/0.16.1/documentation/resources/webcomponents-cover.png)
...
| `bot` | A slash-separated list of [TwicPics API transformations](https://www.twicpics.com/docs/reference/transformations) to be performed for search engine bots. This overrides all other transformations when provided, even if empty (i.e `bot=""`). See the [TwicPics bot attribute documentation](https://www.twicpics.com/docs/reference/script-attributes#data-twic-bot) for more information. | `String` | |
| `duration` | Specifies in seconds the duration extracted from the source video. By default, `duration` is applied from the beginning of the video. You can change this behaviour by using the `from` or `to` properties. Must be a positive number as in `"10.55"` or `10.55`. See [duration documentation](https://www.twicpics.com/docs/reference/transformations#duration) for more information. | `String or number` | |
...
| `focus` | Sets the focus point in `cover` mode. `focus` takes precedence over `anchor` when both are provided. See the [TwicPics focus attribute documentation](https://www.twicpics.com/docs/reference/script-attributes#data-twic-focus) for more information. Only use this attribute if you need a specific focus point or if you want to leverage smart cropping with `focus="auto"`: if you only need border-based positionning (`top`, `bottom`, `left`, `right`, etc), use `anchor` instead. | `String` | |
| `from` | Specifies in seconds the starting position of the video. By default, the video will be sliced up to the end of the source video. You can change this behaviour by using the `duration` or `to` properties. Must be a positive number as in `"5.25"` or `5.25`. See [from documentation](https://www.twicpics.com/docs/reference/transformations#from) tfor more informations. | `String or number` | |
...
| `posterFrom` | Defines the position in seconds of the frame used to generate the poster. Must be a positive number as in `"3.05"` or `3.05`. By default, the poster is generated using the first frame of the video actually played. | `String or number` | |
...
| `to` | Specifies in seconds the ending position of the video. By default, the video will be sliced from the very start of the source video. You can change this behaviour by using the `from` properties. Must be a positive number as in `"9.55"` or `9.55`. See [to documentation](https://www.twicpics.com/docs/reference/transformations#to) for more informations. | `String or number` | |
...
[npm-image]: https://img.shields.io/badge/npm-v0.16.1-orange.svg?style=flat-square
[npm-url]: https://npmjs.org/package/@twicpics/components/v/0.16.1