...
![TwicPics Components](https://raw.githubusercontent.com/twicpics/components/0.26.0/documentation/resources/react-cover.png)
...
In the entry point file of your `React` project.
...
then, use `<TwicImg>`, `TwicPicture` or `<TwicVideo>` in place of standard tags `<img>`, `<picture>` or `<video>` (see [Components Properties](#components-properties)).
...
To activate the [style-driven zoom](#style-driven-approach), simply set the `zoom` property to `'css'` and add a new rule to your stylesheet.
...
For `TwicImg` and `TwicVideo` components, passing a callback function to `onStateChange` gives access to your image's or video's loading state.
...
> [!WARNING]
> When using `onStateChange` in a **Server Components module graph**, your component must be decorated with the [use client directive](https://nextjs.org/docs/getting-started/react-essentials#the-use-client-directive), as following.
...
Each CSS variable corresponds to one of the component attributes listed in the [Components Properties section](#components-properties). If present, the attribute takes precedence over the corresponding CSS variable.
