...
![TwicPics Components](https://raw.githubusercontent.com/twicpics/components/0.26.0/documentation/resources/sveltekit-cover.png)
...
If you don't already have a [layout](https://kit.svelte.dev/docs/routing#layout-layout-svelte) that applies to all the pages of your application,  create it by adding `+layout.svelte` file, right next to your entry point `src/routes/+page.svelte`.
...
For `TwicImg` and `TwicVideo` components, binding `state` props give access to the loading state of your image or video.
...
It is handy when creating a "hero" banner. You can specify the height of your image while respecting its natural aspect ratio and optimizing your _Cumulative Layout Shift_ (CLS) metric.
