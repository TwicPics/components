...
![TwicPics Components](https://raw.githubusercontent.com/twicpics/components/0.19.0/documentation/resources/sveltekit-cover.png)
...
| `refit` | Crops the image as close as possible to the main object(s) in `contain` mode, or maximizing the display area of said object(s) while respecting `ratio` in `cover` mode. Can be `true`, `false` or a list of comma-separated [length](https://www.twicpics.com/docs/reference/parameters#length) defining padding. See the [TwicPics refit documentation](https://www.twicpics.com/docs/reference/transformations#refit) for more information.| `boolean or String ` | `false` |
