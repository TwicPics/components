...
![TwicPics Components](https://raw.githubusercontent.com/twicpics/components/0.19.0/documentation/resources/react-native-cover.png)
...
| `refit` | Resizes the image to maximize the area occupied by the main object(s). Can be `true`, `false` or a list of comma-separated [length](https://www.twicpics.com/docs/reference/parameters#length) defining padding. Preserves `ratio` or gets as close as possible to main object(s) in `cover` and `contain` mode respectively. Note See the [TwicPics refit documentation](https://www.twicpics.com/docs/reference/transformations#refit) for more information.| `boolean or String ` | `false` |
