...
![TwicPics Components](https://raw.githubusercontent.com/twicpics/components/0.9.0-beta.0/documentation/resources/nuxt-cover.png)
...
Thanks to the open source [TwicPics](https://www.twicpics.com/?utm_source=github&utm_medium=organic&utm_campaign=components) Components, delivering responsive images in your [Nuxt3](https://nuxtjs.org/) projects has never been easier.
...
| `anchor` | Allows to define the positioning of an image inside its container in `contain` mode and the focus point in `cover` mode. Syntax is `top`, `bottom`, `left`, `right`, `center` and combinations `left top` or `left-top` and so on. `position` and `focus` properties, in `contain` and `cover` mode respectively, take precedence over `anchor` value. When provided in combination with a `preTransform` value, an optional `focus` value will be applied before the transformation(s) defined with `preTransform`, while `anchor` will be applied just before the final transformation (contain or cover). | `String` | |
| `focus` | Allows to set the focus point before transformation(s) provided with a `preTransform` value or before terminal cover transformation (`mode` = `'cover'`). When provided, `focus` will take precedence over an optional `anchor` value unless `preTransform` is also set. In this case, `focus` will be applied before transformation(s) defined with `preTransform` while `anchor` will be applied just before the final transformation (contain or cover). Can be `auto` or coordinates. See the [TwicPics focus attribute documentation](https://www.twicpics.com/docs/script/attributes#data-twic-focus) for more information. | `String` | |
...
[npm-image]: https://img.shields.io/badge/npm-v0.9.0-beta.0-orange.svg?style=flat-square
[npm-url]: https://npmjs.org/package/@twicpics/components/v/0.9.0-beta.0